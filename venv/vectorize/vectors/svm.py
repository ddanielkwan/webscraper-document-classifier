...
def main():
    ...
    for i in range(len(predictions_NB)):
        print("The actual label : {}  The predicted label : {}".format(y_test.iloc[i],predictions_NB[i]))
    ...
    prediction1 = logi.predict(x_test)
    for y in y_test:
        print("Actual {} :   Predicted : {}".format(y_test.iloc[y],prediction1[y]))
    score = logi.score(x_test,y_test)
    print("Logistic Regression Accuracy Score -> ",score*100)
    """
